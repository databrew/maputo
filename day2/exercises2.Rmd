...
## Part 0 (Categorical with categorical)

A. Run the following to read in some data about people

```{r, eval = FALSE}
people <- read_csv("https://raw.githubusercontent.com/databrew/databrew.github.io/master/people.csv")
```

B. Look at the "cabeça" of `people`. 

C. Create an object called `x`. This should be the number of people for each sex.

D. Create a barchart of the number of people of each sex.

E. Add a variable to `x` using `mutate`. Name this variable `percent`. Make it a percentage of the total number of people (ie, the percent that are women and the percent that are men).

F. Plot the percentage of people in the dataset that are men vs. women.

G. Create a new object called `hm`. This will be `people`, grouped by `health` and `marriage`. Tally (count) the number of people in each group.

H. Make a barplot of the `hm` object. Put `marriage` on the x-axis. Make the `fill` reflect the `health` of the people.

I. Add `position = 'dodge'` to the inside of `geom_bar()` in order to change the look of the chart a little bit.

J. Pick two other categorical variables. Tally the number of people in the groups and make a chart.





## Part 1 (Quantitative with Quantitative)
...
## Part 3 (Scatterplots, misc practice)
